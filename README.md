...
발행 메시지는 큐로 라우팅 되어야합니다. 메시지가 큐로 성공적으로 라우팅되고 추가 전달을 수락할 수 있는 온라인 상태의 컨슈머가 있는 경우 해당 컨슈머에게 전송됩니다.<br/>
...
### AMQP o-9-1
모든 메시지 전송에는 메시지 메타데이터와 전송 정보가 포함됩니다.
클라이언트 라이브러리마다 이러한 소겅에 접근하는 방식이 다릅니다.
아래 속성들은 전달 및 라우팅 세부 정보이며 메시지 자체의 속성은 아니고 RabbitMQ가 라우팅 및 전달 시점에 설정하는 속성입니다.
- delivery tag : 배송 식별자를 의미하고 정수로 값을 사용합니다.
- redelivered : Boolean값을 사용하고, 메시지가 이전에 전달 되었고 다시 대기열에 추가되는 경우 true로 설정합니다.
- exchange : 메시지를 전달하는 거래소 
- routing key : 발행자가 사용하는 라우팅 키
- consumer tag : 소비자 식별자

아래 속성들은 메시지 속성입니다. 대부분은 선택사항이고 발행자가 메시지 발행 시점에 설정합니다.
- delivery mode : 1 또는 2 값을 사용합니다. 2는 디스크에 저장하여 영구적으로 메시지를 저장하고 1은 메모리에 일시적으로 저장합니다. 일부 클라이언트 라이브러리는 이 속성을 bool 또는 열거형으로 노출합니다. 필수로 값을 설정을 해야합니다.
- type : 문자열로 값을 설정하고 하고 애플리케이션별 메시지 유형을 나타낼때 사용합니다. 필수로 값을 설정하지 않아도 됩니다. (e.g. product.buy)
- headers : Map<String, Object> 형식으로 값을 사용하여 속성 설정하고 필수로 값을 설정하지 않아도 됩니다.
- content type : 문자열로 값을 설정하고 애플리케이션에서 application/json과 같이 content type을 설정할때 사용하고 RabbitMQ에서는 사용되지 않습니다. 필수로 값을 설정하지 않아도 됩니다. 
- content encoding : 문자열로 값을 설정하고 애플리케이션에서 png, gzip과 같이 content encoding을 설정할때 사용하고 RabbitMQ에서는 사용되지 않습니다. 필수로 값을 설정하지 않아도 됩니다.
- message id : 임의의 메시지 id를 나타내고 문자열로 값을 설정합니다. 필수로 값을 설정하지 않아도 됩니다.
- correlation id : 문자열로 값을 설정하고 요청과 응답 간의 상관 관계를 파악하는데 도움이 됩니다. 필수로 값을 설정하지 않아도 됩니다.
- reply to : 문자열로 값을 설정하고 응답 큐 이름을 포함하고 필수로 값을 설정하지 않아도 됩니다.
- expiration : 문자열로 값을 설정하고 메시지 만료 시간을 설정할때 사용하고 필수로 값을 설정하지 않아도 됩니다. 
- timestamp : Timestamp로 값을 설정하고 필수로 값을 설정하지 않아도 됩니다.
- user id : 문자열로 값을 설정하고 사용자 id를 나타낼때 설정하고 필수로 값을 설정하지 않아도 됩니다.
- app id : 문자열로 값을 설정하고 애플리케이션이름을 설정하고 필수로 값을 설정하지 않아도 됩니다.

## 메시지 유형(type)
메시지의 type 속성은 애플리케이션이 메시지 종류를 파악하는데 도움이 되는 임의의 문자열입니다.
이 속성은 메시지 발행자가 메시지 발행 시점에 설정합니다. 값은 발행자와 소비자가 정한 문자열이면 어떤것이든 가능합니다.<br/>
RabbitMQ에서는 이 속성을 검증하거나 사용하지 않습니다. 애플리케이션 및 플러그인이 사용하고 해석하기 위해 존재합니다.<br/>
실제 메시지 유형은 장녀스럽게 그룹으로 분류되며 도트(.)로 구분된 명명 규칙이 일반적입니다.<br/>
예로 orders.created 또는 logs.line처럼 명명할 있습니다. RabbitMQ 또는 클라이언트에서 필수 사항은 아닙니다.

## content type, content encoding
컨텐츠 유형 및 인코딩 필드를 통해 발행자는 소비자가 메시지 페이로드를 어떻게 역직렬화하고 디코딩해야하는지 전달할 수 있습니다.
RabbitMQ는 이러한 속성을 검증하거나 사용하지 않고 애플리케이션 및 플러그인이 사용하고 해석하기 위해 존재합니다. <br/>
